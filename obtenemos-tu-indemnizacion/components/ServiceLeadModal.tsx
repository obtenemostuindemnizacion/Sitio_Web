...
export const ServiceLeadModal: React.FC<ServiceLeadModalProps> = ({ isOpen, onClose, service, onNavigate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await saveToGoogleSheets({
      origen: `Formulario Tarjeta - ${service.title}`,
      nombre: formData.name,
      email: formData.email,
      telefono: formData.phone,
      mensaje: `Interés específico en: ${service.title}. Solicitud desde tarjeta de servicios.`
    });
  ...
  return (
    <div className="fixed inset-0 z-[99999] flex items-center justify-center p-0 sm:p-4 overflow-hidden">
      ...
      <div className="relative bg-white w-full max-w-4xl h-full sm:h-auto sm:max-h-[95vh] sm:rounded-[2.5rem] shadow-[0_32px_128px_-16px_rgba(0,0,0,0.5)] overflow-hidden animate-fade-in-up flex flex-col md:flex-row my-auto">
        ...
        {/* Lado Izquierdo: Imagen (Oculta en móvil para priorizar formulario) */}
        <div className="hidden md:flex md:w-2/5 bg-slate-50 relative items-end overflow-hidden border-r border-slate-100">
          <img 
            ...
            alt="Especialista en indemnizaciones" 
        ...
        <div className="w-full md:w-3/5 p-6 md:p-12 overflow-y-auto bg-white flex flex-col">
          {status === 'success' ? (
            <div className="h-full flex flex-col items-center justify-center text-center py-12 my-auto">
              ...
              <h4 className="text-2xl font-black text-slate-900 mb-4 uppercase tracking-tighter">¡Solicitud Recibida!</h4>
              <p className="text-slate-600 mb-8 font-medium">Gracias. Un abogado especialista en <span className="text-brand-600 font-bold">{service.title}</span> contactará contigo en breve para analizar tu caso.</p>
            ...
            <div className="flex flex-col">
              ...
              <h3 className="text-2xl md:text-3xl font-black text-[#2563eb] mb-2 leading-tight text-center md:text-left">
                No pierdas lo que te pertenece por derecho
              </h3>
              <p className="text-slate-600 font-semibold mb-6 text-center md:text-left text-sm">
                Consulta sobre <span className="text-slate-900 font-extrabold uppercase">{service.title}</span> y obtén la máxima compensación.
              </p>
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-start gap-3 px-2 pt-2">
                  <input 
                    type="checkbox" id="privacy-service-modal" required 
                    className="mt-1 w-5 h-5 rounded border-slate-300 text-brand-600 focus:ring-brand-500 cursor-pointer"
                    checked={formData.privacy}
                    onChange={(e) => setFormData({...formData, privacy: e.target.checked})}
                  />
                  <label htmlFor="privacy-service-modal" className="text-[10px] text-slate-500 font-medium leading-tight cursor-pointer">
                ...
                >
                  {status === 'submitting' ? <Loader2 className="w-6 h-6 animate-spin" /> : "Solicitar Información Gratis"}
                </button>
              </form>
